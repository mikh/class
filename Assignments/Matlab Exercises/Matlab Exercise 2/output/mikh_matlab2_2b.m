...
if RUN_PART_B == 1
    ...
    [CCR, TT, ETT] = mikh_svm_cross_validation_rbf(X_a, Y_a ,N_a,5,-5, 15, -13, 3, false, 'RBF', TT, ETT);
    mikh_plot_contour_CCR(2, CCR, 2.^(-5:15),2.^(-13:3),'Binary SVM classifier CCR for classes 1 and 20 using RBF', 'part_b_CR_CCR.png');
    [max_value, max_box_index] = max(CCR);
    [max_value, max_rbf_index] = max(max_value);
    max_box_index = max_box_index(max_rbf_index);
    [CCR, TT, ETT] = mikh_full_SVM(X_a, X_a_test, Y_a, Y_a_test, N_a, N_a_test, 2^(-5+max_box_index-1), 2^(-13+max_rbf_index-1), false, 'RBF', TT, ETT);
