...
if RUN_PART_E == 1
    ...
    if RUN_CLASS_SEPERATION == 1
        classes = cell(20,1);
        class_sizes = zeros(20,1);
        for ii = 1:20
            [classes{ii}, class_sizes(ii), TT, ETT] = mikh_get_class_documents(X_wf_train, Y_train, N_train, ii, TT, ETT);
        end
    end
    [SVM_classifiers, num_classifiers, training_time, TT, ETT] = mikh_train_ovo_svm(classes, class_sizes, 20, 'linear', TT, ETT);
    [CCR, confusion_matrix, testing_time, TT, ETT] = mikh_test_OVO_SVM(SVM_classifiers, num_classifiers, X_wf_test, Y_test, N_test, TT, ETT);
    fprintf('CCR = %.2f\n', CCR);
    fprintf('training time = %.2fs\n', training_time);
    fprintf('testing_time = %.2fs\n', testing_time);
    fid = fopen('part_e_confusion_matrix.txt', 'wt');
