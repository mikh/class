...
if RUN_PART_CD == 1
    ...
    X_a = X_wf_train;
    N_a = N_train;
    [Y_a, TT, ETT] = mikh_get_all_but_class_documents(Y_train, N_train, 17, TT, ETT);
    
    X_a_test = X_wf_test;
    N_a_test = N_test;
    [Y_a_test, TT, ETT] = mikh_get_all_but_class_documents(Y_test, N_test, 17, TT, ETT);

    [CCR, con_mat, TT, ETT] = mikh_svm_cross_validation_balance(X_a, Y_a ,N_a,5,-5, 15, false, 'linear', TT, ETT);
    mikh_plot_CCR(1,CCR,2.^(-5:15),'Binary SVM classifier CCR for classes 17 and the rest', 'part_c_CV_CCR.png');
    mikh_plot_confusion_matrix(2, con_mat, 2.^(-5:15), 'Binary SVM classfier Precision, Recall, and F-Score', 'part_d_CV_CCR.png');
    [max_value, max_index] = max(CCR);
    [CCR, con_mat, TT, ETT] = mikh_full_SVM_balance(X_a, X_a_test, Y_a, Y_a_test, N_a, N_a_test, 2^(-5+max_index-1), 0, false, 'linear', TT, ETT);
